...
export class AcceptRequestComponent implements OnInit {
  ...
  showNotificationDetails() {
    ...
    this.notificationsService.notificationData.userId =
      this.selectedhire.userId;
    ...
    this.notificationsService.notificationData.notification =
      "New offer received from " + this.selectedhire.jobUniqueId;
  }
  ...
  calculateFees() {
    ...
    const platformFee =
      (platformFeePercentage / 100) * this.selectedhire.biddingRate;
  ...
  onSubmitNotification(form: NgForm, userId: string) {
    this.notificationsService.notificationData.notification =
    "Your proposal not accepted from " + this.selectedhire.postBy;
  ...
  RejectNotification() {
    const notificationData = {
      notification: "Your proposal not accepted from " + this.selectedhire.jobUniqueId,
      userId: this.selectedhire.userId,
    };
    this.notificationsService
      .postNotification(this.userId, notificationData)
